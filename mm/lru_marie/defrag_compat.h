...
/*
 * 7.3 split the buddy-allocator's mm-internal API out of mm/internal.h into
 * the new mm/page_alloc.h: post_alloc_hook(), free_pages_prepare(),
 * buddy_order(), buddy_order_unsafe(), pageblock_pfn_to_page(),
 * set_zone_contiguous() and struct alloc_context all live there now.
 * defrag.c uses all of them, so pull the header in on >= 7.3; on older
 * kernels "../internal.h" (included by defrag.c just above us) still has them.
 */
 ...
 * pool back-pressure.
 *
 * 7.3 additionally gave post_alloc_hook() a trailing @alloc_flags argument
 * (used only to feed pgalloc_tag_add()); stock compaction_alloc() passes
 * ALLOC_DEFAULT there, and so do we -- this is the same "hand a raw free page
 * out as a migration target" contract.
 */
