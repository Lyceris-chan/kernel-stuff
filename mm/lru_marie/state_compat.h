...
/*
 * mem_cgroup_swappiness() moved out of <linux/memcontrol.h> and into the
 * mm-private mm/swap.h in 7.3 (the same swap-layer reshuffle that replaced
 * "struct swap_iocb **plug" with "struct swap_io_ctx *ctx" and renamed
 * mm/swap.c to mm/folio.c).  Pull the private header in on >= 7.3 so the
 * reclaim engine keeps calling the identical name on every target; on older
 * kernels <linux/memcontrol.h> (already included by the callers) supplies it.
 */
