...
**Base version:** `7.2.0-2-sleepy`
**Artifact:** `linux-sleepy-7.2.0-2-x86_64.pkg.tar.zst`
...
### 7.3 preview (`sleepy-next`)

The linux-next (7.3 merge-window) preview kernel is a separate package —
`sleepy-next/` (user docs: `sleepy-next/docs/README.md`). The earlier
`wannabe-7.3-rc1` preview tree it replaced was removed on 2026-09-02.
...
- `linux-sleepy-7.2.0-2-x86_64.pkg.tar.zst`
- `linux-sleepy-headers-7.2.0-2-x86_64.pkg.tar.zst`
...
```bash
sudo pacman -U linux-sleepy-7.2.0-2-x86_64.pkg.tar.zst \
              linux-sleepy-headers-7.2.0-2-x86_64.pkg.tar.zst
```
...
The series carries 174 patches in the ranges below. `PATCH_SOURCES.md` is the
