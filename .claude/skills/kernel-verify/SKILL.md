...
|---|---|---|---|
| `fast` | seconds | nothing | `source=()` files exist; no two patches share a number; `b2sums` current; patch headers intact and no `[PATCH n/N]`; each patch carries a real commit id or none; every patch number documented in `PATCH_SOURCES.md`; documented patch count and base version match the PKGBUILD; skills satisfy the Agent Skills spec; no dangling symlinks |
...
- **Identifier sanity** — the `From <id> Mon Sep 17` line is only checked for
  its *shape*, because `patch headers` above asks merely whether `From:` and
  `Subject:` are present. An extraction tool wrote the all-zeros object name
  into 23 patches at once and nothing noticed; `1145` carries a 41-character
  value, and three patches read `From nobody`.
