...
description: >
  Refresh the CachyOS per-branch patch set from sirlucjan's separated patch
  directories. Use when the linux-next snapshot bumps, when sirlucjan releases
  updated branch patches, or when a conflict is reported in a CachyOS branch
  patch. The series base is a linux-next snapshot (see the reference-tree note
  below); this package carries only the on-target squashes (0101–0103 +
  curated fork backports 0110–0112) — 0106/0107/0109 and the old 0151/0053
  conflict handling were superseded by the linux-next base.

  NOTE: `0110`–`0112` (CONFIG_CACHY config hooks, ACPI BM check, amdgpu S5
  eviction skip) are curated backports from the CachyOS/linux fork, NOT
  sirlucjan branch squashes — they must NOT be regenerated by this skill's
  sirlucjan workflow.

  **Reference tree (2026-09-02):** the 7.2 `linux-sleepy` base is retired.
  Generate/check squashes against a worktree of `repos/linux-next` at the
  PKGBUILD `_srctag`; `repos/linux-7.2*` mentions below are historical.
---
