...
- Test a patch independently against the clean tree:
  ```bash
  git -C repos/linux-7.2-rc7 apply --check "$PWD/patches/<range>/<patch>"     # forward (use ABSOLUTE path — git -C changes CWD)
  git -C repos/linux-7.2-rc7 apply --check -R "$PWD/patches/<range>/<patch>"  # already-applied?
  patch -p1 --forward --dry-run < patches/<range>/<patch>                     # THE authoritative check — matches prepare()'s tool
  ```
